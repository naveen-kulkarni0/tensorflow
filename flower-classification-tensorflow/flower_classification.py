...
Original file is located at
    https://colab.research.google.com/drive/1xasnEq-2drYvrNBYUhk6_pLFPcoWaWGz
...
#custom settings
#configuring matplotlib to use tkinter
matplotlib.use('TkAgg')
#setting the tf_XLA flag for gpu
...
# train_data_gen = image_gen.flow_from_directory(train_dir,batch_size=batch_size);
...
# image_gen = ImageDataGenerator(rescale=1./255,rotation_range=80)
...
# image_gen = ImageDataGenerator(rescale=1./255,zoom_range=0.65)
...
#an alternative is to use  tf.keras.preprocessing.image_dataset_from_directory. THis has a property called class_names which will list subdirs in alphabetic order
train_data_gen = image_gen_train.flow_from_directory(train_dir,batch_size=batch_size,class_mode='sparse',target_size=(IMG_SHAPE,IMG_SHAPE))
...
val_data_gen = image_gen_val.flow_from_directory(val_dir,batch_size=batch_size,shuffle=False,class_mode='sparse',target_size=(IMG_SHAPE,IMG_SHAPE))
...
"""
#Using transfer model. Mobilenet is yet another google proejct for high processing on low end devices
IMAGE_RES=224
...
feature_extractor.trainable = False # it is not required as the default value of hub.KerasLayer().trainable is false


#Attach a classification head, basically wrap the hub with output layer
...
# #adding Dense layer I/P
# model.add(tf.keras.layers.Dense(512,activation=tf.nn.relu))

# #adding Dense O/P layer
# model.add(tf.keras.layers.Dense(5,activation=tf.nn.softmax))

#we can also use model.add()
...
# history = model.fit(train_data_gen,batch_size=batch_size,epochs=epochs,steps_per_epoch=math.ceil(len(os.listdir(train_dir))/batch_size),validation_data=val_data_gen,validation_steps=math.ceil(len(os.listdir(val_dir))/batch_size))
#without using steps per epoch and validation steps
history = model.fit(train_data_gen,batch_size=batch_size,epochs=epochs,validation_data=val_data_gen)
...
test_data = test_dir.flow_from_directory(test_path,target_size=(IMG_SHAPE,IMG_SHAPE),shuffle=False)
